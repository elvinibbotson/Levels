...
function backup() {
	...
	request.onsuccess = function(event) {  
    	...
    	if(cursor) {
			...
			window.localStorage.setItem('LossesSave',lastSave); // remember month of backup
		}
	}
}
...
lastSave=window.localStorage.getItem('LossesSave'); // get month of last backup
console.log('lastSave: '+lastSave);
var request=window.indexedDB.open("LossesDB",2);
request.onsuccess=function(event) {
